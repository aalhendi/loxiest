...
impl Scanner {
    ...
    fn identifier_type(&self) -> TokenType {
        match self.source.as_bytes()[self.start as usize] as char {
            'a' => self.check_keyword(1,  "nd", TokenType::And),
            'c' => self.check_keyword(1,  "lass", TokenType::Class),
            'e' => self.check_keyword(1,  "lse", TokenType::Else),
            'f' => {
                if self.current - self.start > 1 {
                    match self.source.as_bytes()[self.start as usize + 1] as char {
    ...
    fn advance(&mut self) -> char {
        ...
        self.source.as_bytes()[self.current as usize - 1] as char
    }
    ...
    fn check_keyword(
        ...
        if &self.source[self.start as usize + start as usize..self.current as usize] == rest {
            kind
        } else {
            TokenType::Identifier
        }
    ...
    fn peek_next(&self) -> Option<char> {
        ...
        Some(self.source.as_bytes()[self.current as usize + 1] as char)
    }
}
